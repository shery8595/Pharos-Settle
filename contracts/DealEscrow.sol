...
contract DealEscrow is ReentrancyGuard {
    ...
    event DeliveryRejected(uint256 indexed dealId, address indexed payer);
    ...
    function rejectDelivery(uint256 dealId) external nonReentrant onlyRouter {
        Deal storage deal = deals[dealId];
        require(deal.state == DealState.Accepted, "bad state");
        require(deal.requiresHybridRelease, "not hybrid");
        ...
        emit DeliveryRejected(dealId, deal.payer);
