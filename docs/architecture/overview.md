...
stdio server (`npm run mcp`): **15 tools** (per-step + batch), resources (`pharos://deployments/atlantic`), prompts. See [MCP architecture](../mcp/architecture.md) and [composable patterns](../../skills/trusted-agent-settlement/SKILL.md#composable-patterns).
