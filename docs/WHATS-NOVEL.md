...
Pharos Settle is **not** generic escrow with a README. The novelty is an **agent commerce layer** on Pharos Atlantic: simulate-first flows, `nextAction` loops, dual-ghost protection, manifest handoff for split identities, and SALI FastPay batch payroll — exposed through **15 MCP tools** and a portable **Skill**.

> **Terminology:** **Dual-ghost protection** is the umbrella term (payee ghosts + payer ghosts). Escrow mechanics exist elsewhere; what ships here is the agent-native orchestration surface.
...
|-----------|------------|-----------------|
| **Dual-ghost protection** | Payee ghosts → payer reclaims (`safetyNet` / `reclaim_trusted_settlement`). Payer ghosts → payee still paid (auto-release + claim). | Neither side can rug the other mid-deal — the core trust hook for agent-to-agent hiring. |
...
| **`preflightHash` binding** | Simulated checks are hashed and stored on-chain with the funded deal. | Funded terms match what was dry-run — no bait-and-switch after preflight. |
...
| Approach | Agent-native API | Ghost payee (no delivery) | Ghost payer (no attest) | Simulate-first | Split identity (payer MCP ≠ payee MCP) | Batch payroll |
|----------|------------------|---------------------------|-------------------------|----------------|----------------------------------------|---------------|
...
| **Pharos Settle** | ✅ Skill + 15 MCP tools + SDK | ✅ `reclaim_trusted_settlement` | ✅ auto-release + claim | ✅ `simulate_trusted_settlement` | ✅ manifest handoff | ✅ SALI FastPay |
...
| Artifact | Link |
|----------|------|
| Contract tests | `npm test` — 103 tests (37 Hardhat + 66 Vitest) — [docs/tests/README.md](tests/README.md) |
