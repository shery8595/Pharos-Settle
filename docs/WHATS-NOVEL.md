...
Pharos Settle is **not** generic escrow with a README. The novelty is an **agent commerce layer** on Pharos Atlantic: simulate-first flows, `nextAction` loops, dual-ghost protection, manifest handoff for split identities, and SALI FastPay batch payroll — exposed through **16 MCP tools** and a portable **Skill**.
...
|-----------|------------|-----------------|
| **Dual-ghost protection** | Payee ghosts → payer reclaims. Payer ghosts → payee still paid (auto-release + claim). Junk delivery → payer rejects during dispute window (cooperative review). | Ghost paths are symmetric; **payer rejection is unilateral** in Phase 1 — see [threat-model.md](security/threat-model.md#payer-rejection-rug-vector-asymmetric-power). |
...
| **Pharos Settle** | ✅ Skill + 16 MCP tools + SDK | ✅ `reclaim_trusted_settlement` | ✅ auto-release + claim | ✅ `reject_delivery` | ✅ `simulate_trusted_settlement` | ✅ manifest handoff | ✅ SALI FastPay |
...
| Artifact | Link |
|----------|------|
| Contract tests | `npm test` — 130 tests (44 Hardhat + 86 Vitest) — [docs/tests/README.md](tests/README.md) |
